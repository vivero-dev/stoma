...
```typescript
import { createGateway, rateLimit, requestValidation, timeout } from "@vivero/stoma";
...
<Aside type="tip">
  For a full, high-quality markdown extraction implementation, see the [packages/gateway/examples/cloudflare-worker/src/index.ts](https://github.com/vivero-dev/stoma/blob/main/packages/gateway/examples/cloudflare-worker/src/index.ts) file in the repository.
</Aside>
...
```sh
npm --prefix examples/cloudflare-worker install @cloudflare/puppeteer
```
...
This recipe is implemented in `examples/cloudflare-worker/src/index.ts`.
