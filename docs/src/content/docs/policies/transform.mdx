...
Applies CORS behavior (simple requests and preflight) as a policy.

```typescript
import { cors } from "@vivero/stoma";
```
...
```typescript
import { overrideMethod } from "@vivero/stoma";
```
...
```typescript
import { requestTransform } from "@vivero/stoma";
```
...
```typescript
import { responseTransform } from "@vivero/stoma";
```
