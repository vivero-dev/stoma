...
draft: false
description: Complete reference for GatewayConfig, RouteConfig, PipelineConfig, and UpstreamConfig types.
sidebar:
  order: 6
---
...
external sources), use the Zod-based validation from `@vivero/stoma/config`:
...
SSRF protection and hop-by-hop headers are documented in [Upstreams](/concepts/upstreams#url-upstream).
...
The binding must be configured in the consuming Worker's `wrangler.jsonc`.

```typescript
interface ServiceBindingUpstream {
  type: "service-binding";

  /** Name of the Service Binding as declared in wrangler.jsonc (e.g. "AUTH_SERVICE"). */
  service: string;
...
The `PolicyContext` interface is documented in [Policy System](/concepts/policies#policy-context).
...
Admin endpoints are documented in [Architecture](/concepts/architecture#admin-introspection-api).
...
- [Policies](/policies/) -- browse all built-in policies and learn how to
