...
`createPolicyTestHarness()` from `@vivero/stoma/sdk` creates a minimal Hono app with everything a policy needs to run:
...
import { InMemoryRateLimitStore, rateLimit } from "@vivero/stoma";
