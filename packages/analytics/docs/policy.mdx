...
Both policies write to `console.log`. Your log aggregation service captures all log lines and delivers them to storage. The downstream [processor](/analytics/architecture/) separates them by `_type` — only lines with `_type: "stoma_analytics"` are extracted into Parquet.
