...
const USERS = [
  { id: "usr_1", name: "Alice Johnson", email: "alice@example.com", role: "admin" },
  { id: "usr_2", name: "Bob Smith", email: "bob@example.com", role: "member" },
  { id: "usr_3", name: "Carol Williams", email: "carol@example.com", role: "member" },
  { id: "usr_4", name: "Dave Brown", email: "dave@example.com", role: "viewer" },
];

function usersHandler(c: Context) {
  const url = new URL(c.req.url);
  const segments = url.pathname.replace(/^\/demo-api\/users\/?/, "").split("/").filter(Boolean);

  if (segments.length > 0) {
    const user = USERS.find((u) => u.id === segments[0]);
    if (user) return c.json(user);
    return c.json({ error: "not_found", message: `User ${segments[0]} not found` }, 404);
  }
...
const PRODUCTS = [
  { id: "prod_1", name: "API Gateway Pro", price: 99.99, category: "software", inStock: true },
  { id: "prod_2", name: "Edge Runtime", price: 49.99, category: "software", inStock: true },
  { id: "prod_3", name: "Rate Limiter Plus", price: 29.99, category: "addon", inStock: false },
  { id: "prod_4", name: "Cache Accelerator", price: 79.99, category: "addon", inStock: true },
  { id: "prod_5", name: "Auth Shield", price: 149.99, category: "security", inStock: true },
];

function productsHandler(c: Context) {
  const url = new URL(c.req.url);
  const segments = url.pathname.replace(/^\/demo-api\/products\/?/, "").split("/").filter(Boolean);

  if (segments.length > 0) {
    const product = PRODUCTS.find((p) => p.id === segments[0]);
    if (product) return c.json(product);
    return c.json({ error: "not_found", message: `Product ${segments[0]} not found` }, 404);
  }
...
function statusHandler(c: Context) {
  ...
  if (Number.isNaN(code) || code < 100 || code > 599) {
    return c.json({ error: "invalid_status", message: "Status code must be 100-599" }, 400);
  }
  return c.json({ status: code, message: `Responded with status ${code}` }, code as 200);
}
...
async function delayHandler(c: Context) {
  ...
  if (Number.isNaN(ms) || ms < 0 || ms > 10000) {
    return c.json({ error: "invalid_delay", message: "Delay must be 0-10000ms" }, 400);
  }
