...
`@homegrower-club/stoma` compiles a declarative TypeScript configuration object into an HTTP application with priority-ordered policy middleware chains and upstream dispatching. This page describes the core abstractions, the request lifecycle, and the module boundaries that keep the system extensible.
...
**Policy** -- A named middleware handler with a numeric priority. Policies are the building blocks of pipelines. They can inspect, transform, short-circuit, or augment requests and responses. Stoma ships a comprehensive set of built-in policies across multiple categories, plus a Policy SDK for custom policy development. See the [Policy System](/concepts/policies/) page for details.
...
1. **Request ingress.** A client HTTP request arrives at the gateway. Hono's router matches it against registered routes (including the gateway's `basePath` prefix).
...
5. **Policy pipeline execution.** Global policies (from `GatewayConfig.policies`) and route-level policies (from `PipelineConfig.policies`) are merged by name -- route-level wins on collision -- and sorted by priority ascending (lower numbers run first). Each policy executes as middleware. Per-policy execution duration is automatically recorded and accumulated on the context under the `_policyTimings` key.
...
|
+-- policies/            Self-contained policy implementations
...
|   +-- cloudflare.ts    Cloudflare KV and Cache API stores, waitUntil, dispatchBinding
