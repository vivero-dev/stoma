...
<Code theme="github-dark-default" code={basicGateway} lang="ts" title="src/gateway.ts" />
<EditorLink code={basicGateway} title="Basic Gateway" />
...
| `routes[].path` | URL pattern to match | `"/*"` or `"/users/*"` |
| `pipeline.policies` | Middleware for this route | `[jwtAuth(), rateLimit()]` |
| `upstream.target` | Destination origin | `"https://api.example.com"` |
| `upstream.rewritePath` | Path transformation | `(path) => path.replace("/api", "/v1")` |
