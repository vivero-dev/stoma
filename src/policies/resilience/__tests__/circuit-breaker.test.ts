...
describe("circuitBreaker", () => {
  ...
  describe("store failure resilience", () => {
    function createCrashingStore(methods: Partial<Record<keyof import("../circuit-breaker").CircuitBreakerStore, "throw" | "reject">>) {
      const real = new InMemoryCircuitBreakerStore();
      const handler = (method: keyof import("../circuit-breaker").CircuitBreakerStore) => {
        const mode = methods[method];
        if (mode === "throw") return () => { throw new Error(`${method} crashed`); };
        if (mode === "reject") return () => Promise.reject(new Error(`${method} timeout`));
        return (...args: unknown[]) => (real[method] as (...a: unknown[]) => unknown)(...args);
      };
      return {
        getState: handler("getState") as import("../circuit-breaker").CircuitBreakerStore["getState"],
        recordSuccess: handler("recordSuccess") as import("../circuit-breaker").CircuitBreakerStore["recordSuccess"],
        recordFailure: handler("recordFailure") as import("../circuit-breaker").CircuitBreakerStore["recordFailure"],
        transition: handler("transition") as import("../circuit-breaker").CircuitBreakerStore["transition"],
        reset: handler("reset") as import("../circuit-breaker").CircuitBreakerStore["reset"],
      };
    }
    ...
    it("should fail-open (assume closed) when store.getState() throws", async () => {
      ...
      const { app } = createApp(
        { store: crashingStore, failureThreshold: 5 },
        [{ status: 200 }],
      );
    ...
    it("should not crash when store.recordFailure() throws in closed state", async () => {
      ...
      const { app } = createApp(
        { store: crashingStore, failureThreshold: 1 },
        [{ status: 500 }],
      );
    ...
    it("should still propagate upstream errors when store crashes in catch path", async () => {
      ...
      const app = new Hono();
      const policy = circuitBreaker({ store: crashingStore, failureThreshold: 1 });
      ...
      app.get("/test", () => { throw new Error("upstream exploded"); });
  ...
  describe("onStateChange", () => {
    it("should call onStateChange when circuit transitions from closed to open", async () => {
      ...
      expect(transitions[0]).toEqual({ key: "/test", from: "closed", to: "open" });
    });
    ...
    it("should call onStateChange for open -> half-open -> closed transitions", async () => {
      ...
      expect(transitions[0]).toEqual({ key: "/test", from: "closed", to: "open" });
      expect(transitions[1]).toEqual({ key: "/test", from: "open", to: "half-open" });
      expect(transitions[2]).toEqual({ key: "/test", from: "half-open", to: "closed" });
    ...
    it("should call onStateChange for half-open -> open on failed probe", async () => {
      ...
      expect(transitions).toHaveLength(3);
      expect(transitions[2]).toEqual({ key: "/test", from: "half-open", to: "open" });
