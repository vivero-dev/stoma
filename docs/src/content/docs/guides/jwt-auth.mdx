...
<Code theme="github-dark-default" code={hmacExample} lang="ts" title="examples/jwt-auth/hmac.ts" />
<EditorLink code={hmacExample} title="JWT Auth (HMAC)" />
...
<Code theme="github-dark-default" code={jwksExample} lang="ts" title="examples/jwt-auth/jwks.ts" />
<EditorLink code={jwksExample} title="JWT Auth (JWKS)" />
...
```typescript
import { definePolicy, GatewayError, jwtAuth } from "@vivero/stoma";
