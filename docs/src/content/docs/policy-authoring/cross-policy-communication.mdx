...
Policies in Stoma run as Hono middleware in priority order. They share data
through Hono's built-in context -- `c.set(key, value)` to write and
...
## Hono context as shared state

Every policy handler receives the Hono `Context` object as its first argument.
...
<Aside type="caution">
Hono context values are untyped by default. Always use `as Type | undefined`
