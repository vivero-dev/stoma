...
Policies are the building blocks of Stoma pipelines. Every policy is a named Hono middleware with a numeric priority that determines its execution order within the pipeline. Stoma ships dozens of built-in policies across multiple categories, plus a Policy SDK for authoring custom policies.
