...
`@vivero/stoma` compiles a declarative TypeScript configuration object into an HTTP application with priority-ordered policy middleware chains and upstream dispatching. This page describes the core abstractions, the request lifecycle, and the module boundaries that keep the system extensible.
